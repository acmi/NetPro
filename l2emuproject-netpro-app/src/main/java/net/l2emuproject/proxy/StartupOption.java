...
/**
 * Specifies an argument supported by the {@link NetPro} class.
 * 
 * @author savormix
 */
public enum StartupOption
{
	...
	DISABLE_UI("disables the graphical user interface", "-du", "--disable-ui"),
	...
	@SuppressWarnings("javadoc") HIDE_LOG_CONSOLE(true, "does not show console in GUI", "-hl", "--hide-log-console"),
	// very handy when you want to keep incomplete options
	// in order to switch between UI only/mixed modes etc.
	@SuppressWarnings("javadoc") IGNORE_UNKNOWN(true, "does not warn about unrecognized command line arguments", "-iu", "--ignore-unrecognized-args");
	
	private final boolean _hidden;
	private final String _description;
	private final String[] _alias;
	
	private StartupOption(String description, String... aliases)
	{
		this(false, description, aliases);
	}
	
	private StartupOption(boolean hidden, String description, String... aliases)
	{
		_hidden = hidden;
	 ...
	 */
	public static StartupOption getByAlias(String alias)
	{
		return CACHE.getLookup().get(alias.toLowerCase(Locale.ENGLISH));
	}
	
	private static final StartupOptionMapper CACHE = new StartupOptionMapper();
}
