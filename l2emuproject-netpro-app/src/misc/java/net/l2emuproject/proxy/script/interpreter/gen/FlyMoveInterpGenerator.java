...
public class FlyMoveInterpGenerator extends EntityInterpGenerator<Map<Integer, String>>
{
	public FlyMoveInterpGenerator(String... args)
	{
		super("flymovename-e.dat.xml", args);
	}
	
	@Override
	public void processEntry(Node entry)
	{
		final Integer k1 = L2XMLUtils.getFirstChildIntegerContent(entry, "path");
		final Integer k2 = L2XMLUtils.getFirstChildIntegerContent(entry, "node");
		
		final String name = L2XMLUtils.getFirstChildStringContent(entry, "name");
	...
	{
		final FlyMoveInterpGenerator gen = new FlyMoveInterpGenerator(args);
		gen.parseClientValues();
		gen.dumpToFile(Paths.get("fly.txt"));
	}
}
